		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:choose>
					...
					<xsl:when test="$amount &gt; 6000 and $amount &lt;= 37000">
						<xsl:value-of select="0.15*($amount - 6000) "/>
					</xsl:when>
					<xsl:when test="$amount &gt; 37000 and $amount &lt;= 80000">
						<xsl:value-of select="4650 + 0.30*($amount - 35000) "/>
					</xsl:when>
					<xsl:when test="$amount &gt; 80000 and $amount &lt;= 180000">
						<xsl:value-of select="17550 + 0.37*($amount - 80000) "/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="54550 + 0.45*($amount - 180000)"/>
					</xsl:otherwise>
				</xsl:choose>				
			</xsl:otherwise>
		</xsl:choose>
